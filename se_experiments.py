...
from helpers.classes import EigenArgs, EnvArgs, ActionNetArgs, ActivationType
...
from models.SeGNN import DenseSEGNN
from utils.transforms import AddSpectralBias
...
class Experiment(object):
    ...
    def run(self) -> Tuple[Tensor, Tensor]:
        ...
        eigen_args = EigenArgs(eigen_dim = self.eigen_dim, model_type=self.eigen_model_type, num_layers=self.env_num_layers, env_dim=self.env_dim,
        ...
        for num_fold in folds:
            ...
            best_losses_n_metrics, edge_ratios =\
                self.single_fold(dataset_by_split=dataset_by_split, eigen_args=eigen_args, env_args=env_args, num_fold=num_fold)
    ...
    def my_collate_fn(data_list):
        # Extract necessary information from each sample
        x_list = [data.x for data in data_list]  # Assuming 'x' is an attribute you want to preserve
        edge_index_list = [data.edge_index for data in data_list]
        Q = [data.Q for data in data_list]
        # ... add more as needed

        # Process the information before creating a batch
        batched_x = torch.stack(x_list, dim=0)  # Assuming 'x' is a tensor
        batched_edge_index = edge_index_list  # Assuming 'edge_index' is a tuple

        # Create a new data object with the preserved information
        batched_data = Data(x=batched_x, edge_index=batched_edge_index, Q=Q)
        # ... add more attributes as needed
        return batched_data


    def single_fold(self, dataset_by_split: DatasetBySplit, eigen_args: EigenArgs, env_args: EnvArgs, num_fold: int) -> Tuple[LossesAndMetrics, OptTensor]:

        model = DenseSEGNN(eigen_args=eigen_args, env_args=env_args, pool=self.pool).to(device=self.device)
    ...
    def train_and_test(self, dataset_by_split: DatasetBySplit, model, optimizer, scheduler, pbar, num_fold: int)\
        ...
        train_loader = DataLoader(dataset_by_split.train, batch_size=self.batch_size, shuffle=True, collate_fn=self.my_collate_fn)
        val_loader = DataLoader(dataset_by_split.val, batch_size=self.batch_size, shuffle=True, collate_fn=self.my_collate_fn)
        test_loader = DataLoader(dataset_by_split.test, batch_size=self.batch_size, shuffle=True, collate_fn=self.my_collate_fn)
    ...
    def train(self, train_loader, model, optimizer):
        ...
        for data in train_loader:
            if self.batch_norm and (data.x.shape[0] == 1 or data.num_graphs == 1):
                continue
            optimizer.zero_grad()
            node_mask = self.dataset.get_split_mask(data=data, batch_size=1,
                                                    split_mask_name='train_mask').to(self.device)  # did change in here set batch_size to 1
            ...
            # forward
            scores =\
                model(data.x.to(device=self.device), edge_index=data.edge_index.to(device=self.device), Q = torch.stack(data.Q).to(device=self.device),
        ...
        for data in loader:
            ...
            scores =\
                model(data.x.to(device=self.device), edge_index=data.edge_index.to(device=self.device),
                      edge_attr=edge_attr, Q = torch.stack(data.Q).to(device=self.device), batch=data.batch.to(device=self.device),
